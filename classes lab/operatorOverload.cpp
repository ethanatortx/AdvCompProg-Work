...
class SALESREC: public SalesRep, public ItemCatalog
{
  public:
    ...
    void operator << (SALESREC s) {
      // printout salesrec (for date, rep, and product)
      cout << s.item;
      cout << s.rep;
      cout << s.date;
    };

    bool operator == (SALESREC s) {
      // compare salesrec (for same date, rep, and item)
      return (item == s.item) && (rep == s.rep) && (date == s.date);
    };

    void operator + (float uAmount) {
